(function () {
    getListPromotionNews();
})();

function getListPromotionNews() {
    requestAjaxGet('/api/ApiEnterprise/getlistpromotionnews/0/9').done(function (data) {
        $('#lst-news').html(loopNews(data.listNewsItems));
        if (data.pageSize > 1) {
...
function getListPromotionNewsWithPagination(pageCurrent) {
    requestAjaxGet('/api/ApiEnterprise/getlistpromotionnews/' + pageCurrent + '/9').done(function (data) {
        $('#lst-news').html(loopNews(data.listNewsItems));
    }).fail();
}
...
function initialPaginationClick() {
    $('#pagination li').click(function () {
        ...
        getListPromotionNewsWithPagination($(this).attr('data-page-current'));
    });
}