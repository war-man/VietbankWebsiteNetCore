...
function getListVietbankNews() {
    requestAjaxGet('/api/ApiAbout/getlistvietbanknews/0/9').done(function (data) {
...
function getListVietbankNewsWithPagination(pageCurrent) {
    requestAjaxGet('/api/ApiAbout/getlistvietbanknews/' + pageCurrent + '/9').done(function (data) {
        $('#lst-news').html(loopNews(data.listNewsItems));
    }).fail();
}
